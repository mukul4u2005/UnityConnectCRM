...
class PdfDirectory extends ChurchInfoReport
{
    ...
    public $_PS = 'Letter';
    public $sSortBy = '';
    private string $sTempleNameEncoded;
    private string $sTempleAddressEncoded;
    private string $sTempleCityEncoded;
    private string $sTempleStateEncoded;
    ...
    public function __construct($nc = 1, $paper = 'letter', $fs = 10, $ls = 4)
    {
        ...
        $this->sTempleNameEncoded = iconv('UTF-8', 'ISO-8859-1', SystemConfig::getValue('sTempleName'));
        $this->sTempleAddressEncoded = iconv('UTF-8', 'ISO-8859-1', SystemConfig::getValue('sTempleAddress'));
        $this->sTempleCityEncoded = iconv('UTF-8', 'ISO-8859-1', SystemConfig::getValue('sTempleCity'));
        $this->sTempleStateEncoded = iconv('UTF-8', 'ISO-8859-1', SystemConfig::getValue('sTempleState'));
    }
    ...
    public function header(): void
    {
        ...
        if (($this->PageNo() > 1) || ($bDirUseTitlePage == false)) {
            ...
            $this->Cell(
                ...
                10,
                $this->sTempleNameEncoded . ' - ' . gettext('Directory'),
                1,
                0,
                'C'
            );
    ...
    public function titlePage(): void
    {
        ...
        $sContact = sprintf(
            "%s\n%s, %s  %s\n\n%s\n\n",
            $this->sTempleAddressEncoded,
            $this->sTempleCityEncoded,
            $this->sTempleStateEncoded,
            SystemConfig::getValue('sTempleZip'),
            SystemConfig::getValue('sTemplePhone')
        );
