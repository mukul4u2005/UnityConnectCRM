    ...
    class PdfTaxReport extends ChurchInfoReport
    {
        ...
        public function finishPage($curY, $fam_ID, $fam_Name, $fam_Address1, $fam_Address2, string $fam_City, string $fam_State, string $fam_Zip, $fam_Country): void
        {
            ...
            if ($remittance === 'yes') {
                ...
                $church_mailing = gettext('Please mail you next gift to ') . SystemConfig::getValue('sTempleName') . ', '
                    . SystemConfig::getValue('sTempleAddress') . ', ' . SystemConfig::getValue('sTempleCity') . ', ' . SystemConfig::getValue('sTempleState') . '  '
                    . SystemConfig::getValue('sTempleZip') . gettext(', Phone: ') . SystemConfig::getValue('sTemplePhone');
                ...
                $curX = 30;
                $curY = 246;
                $this->writeAt(SystemConfig::getValue('leftX') + 5, $curY, SystemConfig::getValue('sTempleName'));
                $curY += SystemConfig::getValue('incrementY');
                if (SystemConfig::getValue('sTempleAddress') != '') {
                    $this->writeAt(SystemConfig::getValue('leftX') + 5, $curY, SystemConfig::getValue('sTempleAddress'));
                    $curY += SystemConfig::getValue('incrementY');
                }
                $this->writeAt(SystemConfig::getValue('leftX') + 5, $curY, SystemConfig::getValue('sTempleCity') . ', ' . SystemConfig::getValue('sTempleState') . '  ' . SystemConfig::getValue('sTempleZip'));
                $curY += SystemConfig::getValue('incrementY');
                if ($fam_Country != '' && $fam_Country != 'USA' && $fam_Country != 'United States') {
